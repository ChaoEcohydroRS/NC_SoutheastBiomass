...
| 3 | **Temporally-stratified biomass-at-risk** | Metrics 1 & 4 split by minimum distance to RS window (2018–2019) | Only concurrent disturbances (distance = 0) can cause systematic directional bias; 1–2 yr disturbances allow partial spectral recovery; 3+ yr disturbances are largely recovered or pre-dated the model imagery. |
...
|----------|-------|---------------|
| Concurrent (0 yr) | 2018, 2019 | Highest — RS imagery captures the disturbed state |
| Near (1–2 yr) | 2016, 2017, 2020, 2021 | Moderate — partial spectral/structural recovery |
| Far (3+ yr) | 2015, 2022 | Lower — canopy partially re-established or change not yet captured |
...
### Concurrent disturbances — RS window 2018–2019 only

Only disturbances that overlap the Sentinel-2 imagery window can produce **directional bias** (as opposed to scatter). All other years contribute only to residual variance.

| Metric | GFW (2018+2019) | LandTrendr (2018+2019) |
|--------|-----------------|------------------------|
| Area fraction | 3.20% | 2.12% |
| Biomass-weighted fraction | 2.94% | 1.76% |
| **Worst-case AGB shift** | **3.05 Mg/ha** | **1.83 Mg/ha** |
| Relative to RMSE | **0.089×** | **0.054×** |

The concurrent worst-case is roughly one-eleventh of RMSE (GFW) or one-nineteenth (LandTrendr). This directly supports the conclusion that temporal mismatch drives residual variance, not systematic bias.
...
| **2018** | **1.70** | **1.26** | **0.74** | **76.5** | **1.30** | **RS window** |
| **2019** | **1.50** | **1.68** | **1.13** | **116.7** | **1.74** | **RS window — high-biomass alert** |
...
**The 2019 RS-window finding is the most important for the reviewer response.** Disturbance concurrent with the Sentinel-2 imagery in 2019 fell in above-average biomass stands (density ratio = 1.13, mean AGB = 117 Mg/ha). This is the scenario most likely to produce a systematic downward bias in modelled AGB. However, the worst-case contribution for 2019 alone is only 1.74 Mg/ha — negligible relative to RMSE. This finding should be explicitly acknowledged in the reviewer response and then quantitatively dismissed.
...
| Proximity | Years | GFW area (%) | GFW WC shift | LT area (%) | LT WC shift |
|-----------|-------|-------------|-------------|------------|------------|
| Concurrent (0 yr) | 2018, 2019 | 3.20% | 3.05 Mg/ha | 2.12% | 1.83 Mg/ha |
| Near (1–2 yr) | 2016, 2017, 2020, 2021 | 5.35% | 5.38 Mg/ha | 2.80% | 2.39 Mg/ha |
| Far (3+ yr) | 2015, 2022 | 2.58% | 2.41 Mg/ha | 1.31% | 0.85 Mg/ha |

Only the Concurrent category can cause systematic directional bias. Near and Far disturbances contribute only to scatter in model residuals because sufficient spectral/structural recovery has occurred or the disturbance predates the imagery.
...
Even the worst-case estimate (GFW, all 8 years, all disturbed pixels → 0 AGB) produces a domain-mean AGB shift of 11.1 Mg/ha — one-third of model RMSE. Restricting to disturbances concurrent with the RS imagery, the worst-case drops to 3.05 Mg/ha (GFW) or 1.83 Mg/ha (LandTrendr).

**4. The GFW 2019 density ratio is the most credible pathway for systematic bias — and it is still negligible.**

The 2019 GFW density ratio of 1.13 (disturbance in above-average biomass stands, mean AGB 117 Mg/ha) represents the single year most capable of introducing downward directional bias into the model. Its worst-case domain-level contribution is 1.74 Mg/ha — 0.051× RMSE. Acknowledging this explicitly in the reviewer response pre-empts follow-up challenges.

**5. Overall conclusion.**

Temporal mismatch contributes to **residual variance** (scatter in plot-level predictions) rather than **systematic bias** (directional offset). Three converging lines of evidence support this:
- Disturbed area is spatially limited (≤11.5% of domain over 8 years)
- Disturbance is concentrated in below-average biomass stands for the early period and throughout LandTrendr
- Even the upper-bound impact on domain-mean AGB is well below model RMSE in every stratification

---

### Suggested reviewer response text

> To quantify the potential impact of temporal mismatch, we generated Global Forest Watch (GFW; Hansen et al. 2013) and LandTrendr-based (Kennedy et al. 2010) forest disturbance layers for 2015–2022 and evaluated disturbance patterns within the 557,121 ha forest modelling domain (Supplementary Fig. S[X]). GFW detected 64,011 ha (11.5%) and LandTrendr detected 34,749 ha (6.2%) of cumulative disturbed area over the study period. Four biomass-weighted metrics were computed to assess the magnitude of potential bias. First, the biomass-weighted disturbance fraction (Σ AGB in disturbed pixels / Σ AGB in all forest pixels) was 10.7% (GFW) and 4.8% (LandTrendr) — lower than the corresponding area fractions, because disturbance preferentially affected below-average-biomass stands (AGB density ratios of 0.51–0.74 for 2015–2018). Second, under the most conservative assumption (all disturbed pixels drop to zero AGB), the worst-case shift in domain-mean AGB is 11.1 Mg/ha (GFW) or 5.0 Mg/ha (LandTrendr) — both well below the model validation RMSE of 34.17 Mg/ha. Third, restricting to disturbances concurrent with the Sentinel-2 imagery window (2018–2019) — the only scenario capable of producing systematic directional bias — the worst-case shift reduces to 3.05 Mg/ha (GFW) or 1.83 Mg/ha (LandTrendr), representing 8.9% and 5.4% of RMSE, respectively. Although GFW detected above-average-biomass disturbance in 2019 (density ratio = 1.13, worst-case 1.74 Mg/ha), this remains negligible at the domain scale. These results confirm that temporal mismatch between FIA measurements (2015–2022) and Sentinel-2 imagery (2018–2019) contributes primarily to residual variance rather than systematic bias.
